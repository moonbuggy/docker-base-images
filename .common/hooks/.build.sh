...
set_repo_data () {
	...
	if [ ! -z "${REPO_TAGS}" ]; then
		...
		eval_param_ifn 'REPO_MINOR_VERSIONS' "parse_version_minor \"${REPO_VERSIONS}\""
		 ...
		 echo "Target repo doesn't appear to exists."
	fi
...
get_target_versions () {
	case "${TARGET_VERSION_TYPE}" in
		...
		custom|*)
			ifFunc 'custom_target_versions' \
				&& target_versions="$(custom_target_versions)" \
  ...
  log_debug "Target minimum: ${min_ver}"
	...
	target_versions="$(echo "${min_ver} ${target_versions}" | xargs -n1 | sort -uV | xargs)"
	add_param "$(echo "${min_ver} ${target_versions##*${min_ver} }" | xargs -n1 | sort -uV | xargs)" 'TARGET_VERSIONS'
